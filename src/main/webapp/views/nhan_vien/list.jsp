        ...
        <div class="border border-1 rounded p-3 mt-2">
          ...
          <div class="mt-2">
            ...
            <table class="mt-2 table table-bordered table-hover">
              ...
              <tbody>
                <c:forEach items="${data}" var="nhanVien" varStatus="items">
                  <tr>
                    <td>${items.index + 1}</td>
                    <td>${nhanVien.id}</td>
                    <td>${nhanVien.ma}</td>
            ...
            <nav aria-label="Page navigation">
              <ul class="pagination justify-content-center">
                <c:if test="${currentPage > 1}">
                  <li class="page-item">
                    <a class="page-link"
                      href="/nhan-vien/${not empty keyword ? 'search?keyword=' += keyword += '&' : 'index?'}page=${currentPage - 1}">Previous</a>
                  </li>
                </c:if>
                ...
                <c:forEach begin="1" end="${totalPages}" var="i">
                  <c:choose>
                    ...
                    <c:otherwise>
                      <li class="page-item">
                        <a class="page-link"
                          href="/nhan-vien/${not empty keyword ? 'search?keyword=' += keyword += '&' : 'index?'}page=${i}">${i}</a>
                      </li>
                    </c:otherwise>
                  </c:choose>
                </c:forEach>
                ...
                <c:if test="${currentPage < totalPages}">
                  <li class="page-item">
                    <a class="page-link"
                      href="/nhan-vien/${not empty keyword ? 'search?keyword=' += keyword += '&' : 'index?'}page=${currentPage + 1}">Next</a>
                  </li>
                </c:if>
              </ul>
            </nav>
