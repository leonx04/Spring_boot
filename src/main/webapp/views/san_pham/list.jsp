...
<head>
  <title>Sản phẩm | Danh sách sản phẩm</title>
...
<div class="container">
  <div class="border border-1 rounded p-3 mt-4">
    <h2 class="text-center mt-2">Danh sách sản phẩm</h2>
    <div class="mt-2">
      <div class="mt-2 w-25 ">
        <!-- Thêm form tìm kiếm -->
        <form action="/san-pham/search?page=${currentPage}&limit=${data.size}" method="GET" class="mb-3">
      ...
      <table class="table table-bordered table-hover">
        ...
        <tbody>
        <c:forEach items="${data.content}" var="sanPham" varStatus="items">
          <tr>
            <td class="fw-bold">${items.index + 1}</td>
            <td>${sanPham.id}</td>
            <td>${sanPham.ma}</td>
            <td>${sanPham.ten}</td>
            <td class="${sanPham.trangThai == 1 ? 'text-success' : 'text-danger'}">
                ${sanPham.trangThai == 1 ? "Đang hoạt động" : "Ngừng hoạt động"}
            </td>
            <td class="text-nowrap">
              <a href="/san-pham/edit/${sanPham.id}" class="btn btn-warning">Sửa</a>
              <a href="/san-pham/delete/${sanPham.id}" class="btn btn-danger">Xoá</a>
            </td>
          </tr>
        </c:forEach>
        </tbody>
      ...
      <nav aria-label="Page navigation">
        <ul class="pagination justify-content-center">
          <li class="page-item ${currentPage == 1 ? 'disabled' : ''}">
            <a class="page-link" href="/san-pham/search?page=${currentPage - 1}&limit=${data.size}&keyword=${keyword}" aria-label="Previous">
              <span aria-hidden="true">&laquo;</span>
            </a>
          </li>
          <li class="page-item ${currentPage == 1 ? 'disabled' : ''}">
            <a class="page-link" href="/san-pham/search?page=1&limit=${data.size}&keyword=${keyword}" aria-label="First">
              Trang đầu
            </a>
          </li>
          ...
          <c:if test="${currentPage > 1}">
            <li class="page-item">
              <a class="page-link" href="/san-pham/search?page=${currentPage - 1}&limit=${data.size}&keyword=${keyword}">${currentPage - 1}</a>
            </li>
          </c:if>
          ...
          <li class="page-item ${currentPage == totalPages ? 'disabled' : ''}">
            <a class="page-link" href="/san-pham/search?page=${totalPages}&limit=${data.size}&keyword=${keyword}" aria-label="Last">
              Trang cuối
            </a>
          </li>

          <li class="page-item ${currentPage == totalPages ? 'disabled' : ''}">
            <a class="page-link" href="/san-pham/search?page=${currentPage + 1}&limit=${data.size}&keyword=${keyword}" aria-label="Next">
